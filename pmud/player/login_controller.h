...
namespace primordia::mud::player {
  ...
  behavior LoginController(stateful_actor<LoginState>* self, strong_actor_ptr command_actor) {
    ...
    return {
      [=](StartLogin) { self->send(actor_cast<actor>(self->state.command_actor), Prompt_v, "enter username"); },
      [=](UserInput, string input) {
        if (self->state.username.empty()) {
          LOG_INFO("Got username {}", input);
          self->state.username = input;
          self->send(actor_cast<actor>(self->state.command_actor), Emit_v, SUPPRESS_ECHO_SEQUENCE);
          self->send(actor_cast<actor>(self->state.command_actor), Prompt_v, "enter password");
        } else {
          LOG_INFO_1("Got password!");
          self->state.password = input;
          self->send(actor_cast<actor>(self->state.command_actor), Emit_v, ENABLE_ECHO_SEQUENCE);
          self->send(actor_cast<actor>(self->state.command_actor), Emit_v, fmt::format("Welcome {}!", self->state.username));
          self->send(actor_cast<actor>(self->state.command_actor), EndLogin_v);
        }
      },
    };
  }
