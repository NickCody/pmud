    ...
    behavior make_behavior() {
      return {
        [this](StartServer) -> int {
          spdlog::info("{} server starting up...", m_mud->get_config().name);

          // ZMQ:
          m_sockfd = socket(AF_INET, SOCK_STREAM, 0);
          if (m_sockfd == -1) {
            spdlog::info("Failed to create socket. errno: {}", errno);
            return errno;
          }
          ...
          if (bind(m_sockfd, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) < 0) {
            spdlog::error("Failed to bind to port {}. errno: {}", m_mud->get_config().port, errno);
            return errno;
          }

          char sockaddr_buffer[INET_ADDRSTRLEN];
          inet_ntop(AF_INET, &sockaddr.sin_addr, sockaddr_buffer, sizeof(sockaddr_buffer));
          spdlog::info("Listening on {}:{}", sockaddr_buffer, m_mud->get_config().port);

          // Start listening. Hold at most 10 connections in the queue
          if (listen(m_sockfd, m_mud->get_config().max_queued_connections) < 0) {
            spdlog::error("Failed to listen on socket. errno: {}", errno);
            return errno;
          }

          spdlog::info("Listening on sockfd: {}", m_sockfd);
        ...
        [this](AcceptConnection) {
          ...
          if (c_id == -1) {
            if (errno == EWOULDBLOCK || errno == EAGAIN) {
              ...
              spdlog::info("Failed to grab connection. errno: {}", errno);
            }
          } else {
        ...
        [this](GoodbyeServer) -> bool {
          spdlog::info("Server actor terminating...");
